...
from all_pytorch_projects.database.models import SpeechCommands
...
speech_router = APIRouter(prefix='/speech', tags=['Speech Commands'])
...
model_path = BASE_DIR / 'pytorch_model' / 'audio_model.pth'


with open(BASE_DIR / 'pytorch_model' / "audio_classes.json", "r", encoding="utf-8") as f:
    labels = json.load(f)
...
@speech_router.post('/predict_speach/')
async def predict_audio(file: UploadFile = File(..., ), db: Session=Depends(get_db)):
    try:
        ...
        with torch.no_grad():
            ...
            data = SpeechCommands(label=file.filename, predict=label)
            db.add(data)
            db.commit()
            db.refresh(data)
